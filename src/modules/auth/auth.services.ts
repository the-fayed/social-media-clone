...
class AuthService {
  ...
  signup = async (signupBody: SignupBody): Promise<string> => {
    const {email, name, username,password, avatar, cover, city} = signupBody
  ...
  try {
      ...
      const user = (await prisma.user.findUnique({ where: { email: email } })) as ILogin;
  ...
  verifyEmailToken = async(token: string): Promise<string> => {
    try {
      const user = await prisma.user.update({where: {emailVerificationToken: token}, data: {
        emailVerified: true,
  ...
  verifyPasswordResetToken = async(code: string): Promise<string> => {
    try {
      const user = await prisma.user.findFirst({where: {passResetToken: generatePasswordRestToken(code)}});
      if (!user || Number(user.passResetTokenExpire) < Date.now()) throw new ApiError('Invalid or expired reset code', 409)
      await prisma.user.update({where: {id: user?.id}, data: {
