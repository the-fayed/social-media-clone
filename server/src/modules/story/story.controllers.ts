...
class StoryControllers {
  ...
  getAllStories = asyncHandler(async (req: AuthorizationRequest, res, next): Promise<void> => {
    ...
    const results = await this.storyServices.getAllStories(userId, req.query);
    if (!results) return next(new ApiError("Can not get stories at the moment", 500));
    res.status(200).json({ status: "success", paginationResult: results.paginationResult, data: results.stories });
  });
